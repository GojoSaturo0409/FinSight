...
@router.put("/feedback")
def submit_categorization_feedback(transaction_id: str, merchant: str, correct_category: str):
    """
    Provide user corrections to the model to continuously fine-tune the ML Categorization weights.
    """
    if isinstance(_default_service.strategy, MLStrategy):
        _default_service.strategy.update_model(merchant, correct_category)
        return {"status": "success", "message": "ML model successfully updated with new weights."}
    return {"status": "skipped", "message": "Feedback loop is only active when MLStrategy is in use."}
